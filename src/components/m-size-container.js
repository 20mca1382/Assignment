...
const MSizeContainer = ({
  ...
  const progressIcon1Style = useMemo(() => {
    return {
      left: propLeft,
    };
  }, [propLeft]);
  ...
  return (
    <div className="rounded-base w-[220px] overflow-hidden flex flex-row items-center justify-start gap-[18px] text-center text-5xl text-gray-200 font-headline-h52">
      <div className="relative w-[88px] h-[88px] shrink-0">
        <div className="absolute w-[70.85%] top-[calc(50%_-_18px)] left-[14.77%] font-light inline-block h-[37.41px]">
          <span className="tracking-[0.11px]">{progressText}</span>%
        </div>
        <img
